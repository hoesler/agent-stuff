...
The default is deliberately generous, because the gap it must clear is not the typical pause between events but the worst legitimate one: reasoning a provider does server-side without streaming deltas, time to first token under load, and a retry's backoff. Erring high costs a deadlock a slower death; erring low costs work in progress.
