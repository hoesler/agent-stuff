 ...
 * - Review a GitHub pull request (checks out the PR locally via gh CLI)
 * - Review an Azure DevOps pull request (checks out the source branch via git fetch)
 ...
 * - `/review gh 123` - review GitHub PR #123 (checks out locally via gh)
 * - `/review gh https://github.com/owner/repo/pull/123` - review GitHub PR from URL
 * - `/review ado 123` - review Azure DevOps PR #123 (auto-discovers org/project/repo from remote)
 * - `/review ado https://dev.azure.com/org/project/_git/repo/pullrequest/123` - review ADO PR from URL
...
type ReviewTarget =
	...
	| { type: "devopsPullRequest"; prId: number; org: string; project: string; repo: string; sourceBranch: string; targetBranch: string; title: string }
...
/**
 * Parse an Azure DevOps PR URL or return null.
 * Supports:
 *   https://dev.azure.com/{org}/{project}/_git/{repo}/pullrequest/{prId}
 *   https://{org}.visualstudio.com/{project}/_git/{repo}/pullrequest/{prId}
 */
function parseAdoPrUrl(ref: string): { org: string; project: string; repo: string; prId: number } | null {
	const trimmed = ref.trim();

	// Modern: https://dev.azure.com/{org}/{project}/_git/{repo}/pullrequest/{prId}
	const modern = trimmed.match(/dev\.azure\.com\/([^/]+)\/([^/]+)\/_git\/([^/]+)\/pullrequest\/(\d+)/i);
	if (modern) {
		return { org: modern[1], project: modern[2], repo: modern[3], prId: parseInt(modern[4], 10) };
	}

	// Legacy: https://{org}.visualstudio.com/{project}/_git/{repo}/pullrequest/{prId}
	const legacy = trimmed.match(/([^.]+)\.visualstudio\.com\/([^/]+)\/_git\/([^/]+)\/pullrequest\/(\d+)/i);
	if (legacy) {
		return { org: legacy[1], project: legacy[2], repo: legacy[3], prId: parseInt(legacy[4], 10) };
	}
...
/**
 * Auto-discover org, project, repo from git remote URL.
 * Returns null if the remote is not an Azure DevOps remote.
 */
function parseAdoRemote(remoteUrl: string): { org: string; project: string; repo: string } | null {
	const url = remoteUrl.trim();

	// Modern HTTPS: https://dev.azure.com/{org}/{project}/_git/{repo}
	const modern = url.match(/dev\.azure\.com\/([^/]+)\/([^/]+)\/_git\/([^/]+)/i);
	if (modern) return { org: modern[1], project: modern[2], repo: modern[3] };

	// Legacy HTTPS: https://{org}.visualstudio.com/{project}/_git/{repo}
	const legacy = url.match(/([^.]+)\.visualstudio\.com\/([^/]+)\/_git\/([^/]+)/i);
	if (legacy) return { org: legacy[1], project: legacy[2], repo: legacy[3] };

	// SSH: git@ssh.dev.azure.com:v3/{org}/{project}/{repo}
	const ssh = url.match(/ssh\.dev\.azure\.com(?::v3)?[:/]([^/]+)\/([^/]+)\/([^/]+)/i);
...
/**
 * Parse a bare PR ID (positive integer) or return null.
 */
function parseAdoPrId(ref: string): number | null {
	const num = parseInt(ref.trim(), 10);
	return !isNaN(num) && num > 0 ? num : null;
}

/**
 * Fetch PR details from the Azure DevOps REST API.
 */
async function getAdoPrInfo(
	...
	const url = `https://dev.azure.com/${encodeURIComponent(org)}/${encodeURIComponent(project)}/_apis/git/repositories/${encodeURIComponent(repo)}/pullrequests/${prId}?api-version=7.0`;
	const { stdout, code } = await pi.exec("curl", [
		"--silent", "--fail",
	...
	try {
		const data = JSON.parse(stdout);
		if (!data.title || !data.sourceRefName || !data.targetRefName) return null;
		return {
			title: data.title,
...
/**
 * Fetch and checkout the source branch of an ADO PR.
 */
async function checkoutAdoBranch(pi: ExtensionAPI, sourceBranch: string): Promise<{ success: boolean; error?: string }> {
	const { stderr: fetchErr, code: fetchCode } = await pi.exec("git", [
		"fetch", "origin", `refs/heads/${sourceBranch}:refs/remotes/origin/${sourceBranch}`,
	]);
	if (fetchCode !== 0) {
		return { success: false, error: fetchErr || `Failed to fetch branch '${sourceBranch}'` };
	}

	// Create or reset local branch tracking origin
	const { stderr: checkoutErr, code: checkoutCode } = await pi.exec("git", [
		"checkout", "-B", sourceBranch, `origin/${sourceBranch}`,
	]);
	if (checkoutCode !== 0) {
		return { success: false, error: checkoutErr || `Failed to checkout branch '${sourceBranch}'` };
	}
...
async function buildReviewPrompt(
	...
	switch (target.type) {
		...
		case "devopsPullRequest": {
			const mergeBase = await getMergeBase(pi, target.targetBranch);
			if (mergeBase) {
				return `Review Azure DevOps pull request #${target.prId} ("${target.title}") from '${target.sourceBranch}' into '${target.targetBranch}'. The merge base commit for this comparison is ${mergeBase}. Run \`git diff ${mergeBase}\` to inspect the changes that would be merged. Provide prioritized, actionable findings.`;
			}
			return `Review Azure DevOps pull request #${target.prId} ("${target.title}") from '${target.sourceBranch}' into '${target.targetBranch}'. Start by finding the merge base between the current branch and '${target.targetBranch}' (e.g., \`git merge-base HEAD ${target.targetBranch}\`), then run \`git diff\` against that SHA to see the changes that would be merged. Provide prioritized, actionable findings.`;
		}
...
const REVIEW_PRESETS = [
	...
	{ value: "pullRequest", label: "Review a GitHub pull request", description: "(via gh CLI)" },
	{ value: "devopsPullRequest", label: "Review an Azure DevOps pull request", description: "(via ADO REST API)" },
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	/**
	 * Resolve an Azure DevOps PR reference (bare prId or URL) into a ReviewTarget.
	 * Handles auth, PR info fetch, and source branch checkout.
	 */
	async function resolveAdoPullRequestTarget(
		ctx: ExtensionContext,
		ref: string,
		options: { skipInitialPendingChangesCheck?: boolean } = {},
	): Promise<ReviewTarget | null> {
		if (!options.skipInitialPendingChangesCheck && (await hasPendingChanges(pi))) {
			ctx.ui.notify(ADO_CHECKOUT_BLOCKED_BY_PENDING_CHANGES_MESSAGE, "error");
			return null;
		}

		// Resolve org/project/repo/prId — either from a full URL or bare prId + remote
		let org: string;
		let project: string;
		let repo: string;
		let prId: number;

		const fromUrl = parseAdoPrUrl(ref);
		if (fromUrl) {
			({ org, project, repo, prId } = fromUrl);
		} else {
			const bareId = parseAdoPrId(ref);
			if (!bareId) {
				ctx.ui.notify("Invalid ADO PR reference. Enter a PR number or a full ADO PR URL.", "error");
				return null;
			}
			prId = bareId;

			// Auto-discover org/project/repo from git remote
			const { stdout: remoteUrl, code: remoteCode } = await pi.exec("git", ["remote", "get-url", "origin"]);
			if (remoteCode !== 0 || !remoteUrl.trim()) {
				ctx.ui.notify("Could not determine git remote. Use a full ADO PR URL instead.", "error");
				return null;
			}
			const parsed = parseAdoRemote(remoteUrl.trim());
			if (!parsed) {
				ctx.ui.notify(
					"Git remote does not look like an Azure DevOps remote. Use a full ADO PR URL instead.",
					"error",
				);
				return null;
			}
		...
		// Get auth token
		ctx.ui.notify("Authenticating with Azure DevOps...", "info");
		const token = await getAdoToken(pi);
		if (!token) {
			ctx.ui.notify(`Azure DevOps authentication failed. ${ADO_AUTH_INSTRUCTIONS}`, "error");
			return null;
		}

		// Fetch PR details
		ctx.ui.notify(`Fetching ADO PR #${prId} info...`, "info");
		const prInfo = await getAdoPrInfo(pi, org, project, repo, prId, token);
		if (!prInfo) {
			ctx.ui.notify(
				`Could not fetch ADO PR #${prId}. Check that it exists and your credentials have access.`,
				"error",
			);
			return null;
		}

		// Re-check pending changes right before checkout
		if (await hasPendingChanges(pi)) {
			ctx.ui.notify(ADO_CHECKOUT_BLOCKED_BY_PENDING_CHANGES_MESSAGE, "error");
			return null;
		}
		...
		ctx.ui.notify(`Checking out '${prInfo.sourceBranch}'...`, "info");
		const checkoutResult = await checkoutAdoBranch(pi, prInfo.sourceBranch);
		if (!checkoutResult.success) {
			ctx.ui.notify(`Failed to checkout branch: ${checkoutResult.error}`, "error");
			return null;
		}

		ctx.ui.notify(`Checked out ADO PR #${prId} (${prInfo.sourceBranch})`, "info");
	...
	/**
	 * Show ADO PR input and handle checkout
	 */
	async function showAdoPrInput(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const prRef = await ctx.ui.editor(
			"Enter ADO PR number or URL (e.g. 123 or https://dev.azure.com/org/project/_git/repo/pullrequest/123):",
			"",
		);

		if (!prRef?.trim()) return null;

		return await resolveAdoPullRequestTarget(ctx, prRef, { skipInitialPendingChangesCheck: true });
	}
	...
	type ParsedReviewArgs = {
		target: ReviewTarget | { type: "gh"; ref: string } | { type: "ado"; ref: string } | null;
	...
	function parseArgs(args: string | undefined): ParsedReviewArgs {
		...
		switch (subcommand) {
			...
			case "gh": {
				...
				return { target: { type: "gh", ref }, extraInstruction };
			}

			case "ado": {
				const ref = parts[1];
				if (!ref) return { target: null, extraInstruction };
				return { target: { type: "ado", ref }, extraInstruction };
			}
	...
	/**
	 * Handle GitHub PR checkout and return a ReviewTarget (or null on failure)
	 */
	...
	/**
	 * Handle Azure DevOps PR checkout and return a ReviewTarget (or null on failure)
	 */
	async function handleAdoPrCheckout(ctx: ExtensionContext, ref: string): Promise<ReviewTarget | null> {
		return await resolveAdoPullRequestTarget(ctx, ref);
	}
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			if (parsed.target) {
				if (parsed.target.type === "gh") {
					// Handle GitHub PR checkout (async operation)
					target = await handlePrCheckout(ctx, parsed.target.ref);
					if (!target) {
						ctx.ui.notify("GitHub PR review failed. Returning to review menu.", "warning");
					}
				} else if (parsed.target.type === "ado") {
					// Handle Azure DevOps PR checkout (async operation)
					target = await handleAdoPrCheckout(ctx, parsed.target.ref);
					if (!target) {
						ctx.ui.notify("ADO PR review failed. Returning to review menu.", "warning");
					}
				} else {
