...
| [`hunk`](pi/extensions/hunk) | One `/hunk` command over a live [Hunk](https://www.hunk.dev) diff window: it reviews a changeset and leaves findings as inline notes, or collects the notes you left there and addresses them, replying on each line. Reuses the window you have open — splitting a [herdr](https://herdr.dev) pane or a Ghostty window to make one — and adopts Hunk's own agent skill so its CLI knowledge tracks the installed binary. |
